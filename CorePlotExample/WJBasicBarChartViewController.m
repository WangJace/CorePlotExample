...
#define BarChartColor(a) [CPTColor colorWithComponentRed:245/255.0 green:166/255.0 blue:35/255.0 alpha:a]

@interface WJBasicBarChartViewController ()<CPTBarPlotDelegate, CPTBarPlotDataSource>
{
    NSArray *_dataSource;
}
...
@implementation WJBasicBarChartViewController

- (void)viewDidLoad {
    [super viewDidLoad];
    // Do any additional setup after loading the view from its nib.
    self.view.backgroundColor = [UIColor whiteColor];
    self.navigationItem.title = @"Basic Bar Chart";
...
- (void)setPlot {
    ...
    borderLineStyle.lineColor = BarChartColor(1.0);
    // 初始化plot，传入柱状图的主题色和柱状条的方向(是否为横向柱状条)
    CPTBarPlot *barPlot = [CPTBarPlot tubularBarPlotWithColor:BarChartColor(1.0) horizontalBars:NO];
    barPlot.barWidth = @1;                   // 柱状图的柱宽
    barPlot.barOffset = @0.0;                // 柱状图的柱子向左偏移
...
{
    // 渐变色
    CPTGradient *fillGradient = [CPTGradient gradientWithBeginningColor:BarChartColor(0.8) endingColor:BarChartColor(0.2)];
